...
const handleContinue = async () => {
  ...
  if (existingRole) {
    return navigateTo(existingRole === "admin" ? "/admin" : "/dashboard", {
      replace: true,
    });
  }
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 px-4">
    ...
    <div v-if="!pageReady" class="text-center text-gray-400 text-sm">Loading...</div>
    ...
    <div v-else class="w-full max-w-lg">
        ...
        >
          <div
            :class="[
              ...
              selectedRole === 'user' ? 'bg-teal-200' : 'bg-teal-100',
            ]"
          >
            <svg
      ...
      <button
        ...
        class="w-full py-3 rounded-full font-medium text-white bg-green-600 transition disabled:opacity-30 disabled:cursor-not-allowed hover:bg-green-700"
        @click="handleContinue"
      >
        <span v-if="loading">Processing...</span>
        <span v-else-if="selectedRole === 'user'">Continue as Renter</span>
